...
def register_mock_user(email: str, profile: MockUserProfile):
...
def build_mock_login_router() -> APIRouter:
    ...
    The endpoint:
    1. Verifies ``is_mock_enabled()``
    2. Creates an ``MsGraphInstance`` with mock transport enabled
    3. Acquires a synthetic token (no Azure AD call)
    4. Populates Redis with the mock profile (real Redis, mock data)
    5. Sets ``llming_identity`` + ``llming_auth`` cookies
    6. Redirects to ``/chat``
    """
    router = APIRouter()
    ...
    async def mock_login(email: str, request: Request):
        ...
        if not is_mock_enabled():
            return JSONResponse({"error": "Mock users disabled"}, status_code=403)
        ...
        session_id = f"mock-session-{profile.user_id}"
        ...
        from llming_com.auth import sign_identity_token, sign_auth_token
        ...
        logger.info("[MOCK] Login successful for %s (session=%s)", profile.email, session_id)
        return response
